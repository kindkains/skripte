...
if [ $CERTBOT_EXITCODE -eq 0 ]; then
	...
	certbot_start
	echo "Certbot wurde am $d erfolgreich ausgeführt" | mailx -s "paqo.de cronjob 'Certbot vom $d'" sandro.isoletta@gmail.com
	...
	echo "${T}"
	echo "Certbot wurde am $d fehlerhaft ausgeführt. Bitte überprüfen!" | mailx -s "ERROR: paqo.de cronjob 'Certbot vom $d'" sandro.isoletta@gmail.com
else
	error
	echo "Certbot wurde am $d fehlerhaft ausgeführt. Bitte überprüfen!" | mailx -s "ERROR: paqo.de cronjob 'Certbot vom $d'" sandro.isoletta@gmail.com
fi
